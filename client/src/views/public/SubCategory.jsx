...
export default function SubCategory() {
  ...
  const fetchData = (setItems, items) => {
      axios.get(`http://127.0.0.1:8000/api/posts/sub-categories/${params.id}/articles/paginated?_page=${page}`)
        .then((res) => {
          console.log(res)
  ...
  return (
    <Layout>        
      ...
      <section className="border-top mt-5 pt-2">
        <div className="my-3">
          <h2 className="fw-bolder" style={{color: 'blueviolet'}}>{subCategory.data.title}</h2>
          <p className="fs-5 fw-semibold">{subCategory.data.description}</p>
        </div>
