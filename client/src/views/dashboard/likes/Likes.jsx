...
export default function Likes() {
    ...
    return (
        <Layout>
            <h2 className='my-4 fw-bold' style={{ color: 'blueviolet'}}>Likes</h2>

            <div className="d-grid gap-2 d-md-flex justify-content-md-end mb-3">
                {/* <Link 
                    to={ route('dashboard.likes.create') } 
                        ...
                        return (
                            <li key={like.id} className="list-group-item">
                                <div className="d-grid gap-2 d-md-flex justify-content-end mb-3">
                                    {/* <Link 
                                        to={ route('dashboard.likes.edit', { id: like.id }) }
                                ...
                                <span className='text-secondary fw-semibold'>like by @
                                    <Link 
                                        to={ route('dashboard.authors.show', { id: like.added_by }) }>
                                        <span style={{color: 'blueviolet'}}>{like.added_by}</span>
                                    </Link> | {dayjs(like.created_at).format('MMM D, YYYY')}
                                </span>
                                <div className='d-flex flex-column justify-content-end align-items-end text-end mt-2'>
                                    <span className='fst-italic'>Liked:</span>
                                    <span dangerouslySetInnerHTML={{ __html: like.article.substring(0, 150) }}></span>
                                </div>
