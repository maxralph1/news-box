...
export default function CommentReplies() {
    ...
    return (
        <Layout>
            <h2 className='my-4 fw-bold' style={{ color: 'blueviolet'}}>Comment Replies</h2>

            <div className="d-grid gap-2 d-md-flex justify-content-md-end mb-3">
                {/* <Link 
                    to={ route('dashboard.comments.create') } 
            ...
            <section className='mb-5'>
                <div className="card rounded-0 w-100 shadow">
                    ...
                    <ul className="list-group list-group-flush">
                        {(commentReplies?.length > 0) ? commentReplies.map(commentReply => {
                        return (
                            <li key={commentReply.id} className="list-group-item">
                                <div className="d-grid gap-2 d-md-flex justify-content-end mb-3">
                                    ...
                                    <button 
                                        ...
                                        type="button" 
                                        onClick={ async () => {
                                            await destroyCommentReply(commentReply)
                                ...
                                <span className='text-dark'>
                                    ...
                                    {/* <p dangerouslySetInnerHTML={{ __html: commentReply.body.substring(0, 50) }}></p> */}
                                </span>
                                <span className='text-secondary fw-semibold'>added by @
                                    <Link 
                                        to={ route('dashboard.authors.show', { id: commentReply.added_by }) }>
                                        <span style={{color: 'blueviolet'}}>{commentReply.added_by}</span>
                                    </Link> | {dayjs(commentReply.created_at).format('MMM D, YYYY')}
                                </span>
                                <div className='d-flex flex-column justify-content-end align-items-end text-end mt-2'>
                                    <span className='fst-italic'>Replied to:</span>
                                    {/* <span dangerouslySetInnerHTML={{ __html: commentReply.comment.substring(0, 150) }}></span> */}
                                    <span>{commentReply.comment}</span>
                                </div>
