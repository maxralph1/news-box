...
export default function Comments() {
    ...
    return (
        <Layout>
            <h2 className='my-4 fw-bold' style={{ color: 'blueviolet'}}>Comments</h2>
                        ...
                        return (
                            <li key={comment.id} className="list-group-item">
                                ...
                                <Link 
                                    ...
                                    <h4 className='fw-bold fs-6'>{comment.body.substring(0, 25)}</h4>
                                    {/* <p dangerouslySetInnerHTML={{ __html: comment.body.substring(0, 50) }}></p> */}
                                </Link>
                                <span className='text-secondary fw-semibold'>added by @
                                    <Link 
                                        to={ route('dashboard.authors.show', { id: comment.added_by }) }>
                                        <span style={{color: 'blueviolet'}}>{comment.added_by}</span>
                                    </Link> | {dayjs(comment.created_at).format('MMM D, YYYY')}
                                </span>
                                <div className='d-flex flex-column justify-content-end align-items-end text-end mt-2'>
                                    <span className='fst-italic'>Commented on:</span>
                                    <span dangerouslySetInnerHTML={{ __html: comment.article.substring(0, 150) }}></span>
                                </div>
